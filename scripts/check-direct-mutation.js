...
const repositoryPath = /(business-requirements\.json|software-requirements\.json|\.engine\/)/;
const violations = [];
for (const file of files) {
  if (file.startsWith("src/adapters/repository/")) continue;
  const content = await readFile(file, "utf8");
  // Operations may inspect storage and create an external support artifact,
  // but may not name or access canonical/engine repository paths directly.
  if (file.startsWith("src/operations/") && !repositoryPath.test(content)) continue;
